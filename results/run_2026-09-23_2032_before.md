...
|---|---|---|---|
| How long is the wait at Kestrel Commons during lunch? |   |   |   |
| Is there an enforced quiet time in Morrow House? |   |   |   |
| Can you study during your shift at an on-campus dining job? |   |   |   |
| How often does the campus shuttle run on weekdays? |   |   |   |
| Are the midterms curved in CS 210? |   |   |   |
...
Source: `dining_kestrel_commons.txt` (and similar information in `dining_kestrel_commons_followup.txt`).
```
...
```
Based on the documents provided, the wait time at Kestrel Commons is 20 to 25 minutes between 12:15 and 1:00, and under 5 minutes before 11:45 (*dining_kestrel_commons.txt* and *dining_kestrel_commons_followup.txt*).
```
...
```
Based on the provided documents, the wait time at Kestrel Commons is 20 to 25 minutes between 12:15 and 1:00, and under 5 minutes before 11:45 (*dining_kestrel_commons.txt* and *dining_kestrel_commons_followup.txt*).
```
...
```
Based on the documents, there are no enforced quiet hours in Morrow House (it is loud until about 1am on weekends). 

Sources: `housing_morrow_house_noise.txt` and `housing_morrow_house.txt`
```
...
```
Yes, the midterms are curved in CS 210 (source: `course_cs_210_exams.txt` and `course_cs_210.txt`).
```
...
```
Yes, the midterms are curved in CS 210 (source: `course_cs_210.txt` and `course_cs_210_exams.txt`).
```
